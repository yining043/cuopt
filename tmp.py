...
def routing_solution_to_flat(routing_solution):
    """
    Convert cuopt.routing.Solve output to standard flat format.

    Standard flat format: [0, node1, node2, ..., 0, node3, node4, ..., 0]
    where 0 is depot separator, and nodes are customer IDs (1 to num_orders).

    Args:
        routing_solution: Output from cuopt.routing.Solve

    Returns:
        solution_flat: numpy array in standard flat format, e.g., [0, 1, 2, 3, 0, 4, 5, 6, 0]
    """
    route_data = routing_solution.get_route()

    # Extract data as numpy arrays
    truck_ids = route_data["truck_id"].to_numpy()
    routes = route_data["route"].to_numpy()
    types = route_data["type"].to_numpy()

    # Group nodes by vehicle ID
    vehicle_routes = {}
    for i in range(len(truck_ids)):
        vehicle_id = int(truck_ids[i])
        node = int(routes[i])
        node_type = str(types[i])

        if vehicle_id not in vehicle_routes:
            vehicle_routes[vehicle_id] = []

        # Only include non-depot nodes (skip "Depot" and "Break" types)
        # Depot nodes (node == 0) are added as separators between routes
        if node_type not in ["Depot", "Break"]:
            vehicle_routes[vehicle_id].append(node)

    # Convert to flat format: [0, route1_nodes..., 0, route2_nodes..., 0, ...]
    solution_flat = []

    # Sort by vehicle ID to ensure consistent ordering
    for vehicle_id in sorted(vehicle_routes.keys()):
        route_nodes = vehicle_routes[vehicle_id]
        if len(route_nodes) > 0:
            solution_flat.append(0)  # Start of route (depot)
            solution_flat.extend(route_nodes)  # Customer nodes
            solution_flat.append(0)  # End of route (depot)
