...
# 1. 数据准备 (Data)
# ==========================================
# Ours (Green) - reconstruction_aug_gap, eval_time
gaps_A = np.array([
...
T_MAX = 600.0  # match static plot: Gap (%) vs Time (s)

# Top: high positive gaps; bottom: negative/low gaps
ax1.set_xlim(0, T_MAX)
ax2.set_xlim(0, T_MAX)
ax1.set_ylim(3.0, 50.0)
...
ax2.set_xlabel("Inference Time (seconds)", fontsize=18, fontweight='bold', color="#333333")
fig.text(0.04, 0.5, 'Gap (%)', va='center', rotation='vertical', fontsize=18, fontweight='bold', color="#333333")
fig.suptitle("Performance Trajectories: CaR vs. Baselines", fontsize=20, fontweight='bold', y=0.95)

ax1.tick_params(axis='both', which='major', labelsize=14)
ax2.tick_params(axis='both', which='major', labelsize=14)

# 目标绿区设在底轴 (good gap region)
ax2.fill_between([0, T_MAX], -5.5, 0.5, color='limegreen', alpha=0.15, zorder=1)
ax2.text(10, 0.7, "Desired Region:\nLow gap", color='forestgreen', fontsize=16, fontweight='bold', zorder=2)
...
# Ours
line1_ours, = ax1.plot([], [], color=color_ours, linestyle='-', linewidth=3.5, alpha=1.0, label='CaR (Ours)')
...
ax1.legend(loc='upper right', frameon=True, fontsize=16, edgecolor='gray')
time_text = ax1.text(0.35, 0.85, "", transform=ax1.transAxes, fontsize=20, fontweight='bold', color='#333333',
...
speedup_text = ax2.text(0, 0, "", color='#D35400', fontweight='bold', fontsize=18, ha='center', va='bottom', zorder=10, alpha=0,
...
def update(frame):
    ...
    if len(x_c) > 0:
        ...
        line2_sgbs.set_data(x_c, y_c)
        point1_sgbs.set_offsets([[x_c[-1], y_c[-1]]])
        point2_sgbs.set_offsets([[x_c[-1], y_c[-1]]])
    ...
    time_text.set_text(f"Simulation Time: {current_t:4.0f}s")

    # Speedup: same gap level, compare time (e.g. gap ~ -1.5: Ours much faster)
    if frame >= frames:
        target_gap = -1.5
