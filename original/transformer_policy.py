...
class AutoregressiveDecoder(nn.Module):
    ...
    def forward(self, h, candidate_mask, k_batch, selected_flat=None):
        ...
        for t in range(max_k):
            ...
            if given_sequence is not None:
                ...
                indices = dist.sample()
            log_prob = dist.log_prob(indices) * active_mask.float().squeeze(1)
        ...
        log_probs_total = torch.stack(log_probs, dim=1).sum(dim=1) / k_batch.squeeze(1).float()
...
class TransformerCandidatePolicy(nn.Module):
    def __init__(self, 
        ...
        if use_autoregressive_decoder:
            ...
            raise NotImplementedError("Heat map decoder is not implemented")
    ...
    def forward(self, nodes_tensor, demands_tensor, current_sol_tensor, candidates_tensor, k_batch=None, selected_tensor=None, training=True):
        device = self.device
        ...
        h = torch.cat([h, h[:, :1, :].repeat(1, max_length - N, 1)], dim=1)
        ...
        encoder_mask = torch.arange(max_length, device=device).unsqueeze(0).expand(batch_size, -1) >= num_real_nodes
        if training:
            ...
            candidates_mask = candidates_tensor
        h = self.encoder(h, key_padding_mask=encoder_mask, candidates_mask=candidates_mask)

        # # decoder
        # if training:
        #     candidates_flat = candidates_tensor.view(batch_size * 10, max_length)
        #     selected_flat = selected_tensor.view(batch_size * 10, max_length)
        #     h_expanded = h.unsqueeze(1).expand(-1, 10, -1, -1).contiguous().view(batch_size * 10, max_length, self.d_model)
            
        #     k_batch = selected_flat.sum(dim=1).long()
        #     actions, log_probs = self.decoder(h_expanded, candidates_flat, k_batch, selected_flat)
        #     log_probs = log_probs.view(batch_size, 10)
        # else:
        #     actions, log_probs = self.decoder(h, candidates_tensor, k_batch, None)

                # decoder
        if training:
            candidates_flat = candidates_tensor.view(10, max_length)
            selected_flat = selected_tensor.view(10, max_length)
            h_expanded = h.unsqueeze(1).expand(-1, 10, -1, -1).contiguous().view(batch_size * 10, max_length, self.d_model)
            ...
            sample_size = 10
            # import pdb; pdb.set_trace()
            h_expanded = h.expand(sample_size, -1, -1).contiguous().view(sample_size, max_length, self.d_model)
            candidates_flat = candidates_tensor.expand(sample_size, max_length)
            k_batch = k_batch.expand(sample_size, 1)

            actions, log_probs = self.decoder(h_expanded, candidates_flat, k_batch, None)
            actions = actions.view(sample_size, -1)
            log_probs = log_probs.view(sample_size, -1)
            # import pdb; pdb.set_trace()
            actions_selected = actions[log_probs.argmax(dim=0)]
            log_probs_selected = log_probs[log_probs.argmax(dim=0)]
        return actions_selected, log_probs_selected
