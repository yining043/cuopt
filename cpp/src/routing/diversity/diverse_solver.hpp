...
struct solve {
                      ...
                      bool from_dir           = false)
  {
    ...
    run_ls_and_exit();
    if (!from_dir) {
      printf("Generating solutions from scratch\n");
      generate_from_scratch();
      printf("Solutions generated from scratch\n");
    } else {
    ...
    printf("starting reserve threshold adjustment\n");
    start_reserve_threshold_adjustment();
    printf("running working loop\n");
    run_working_loop();
    printf("done working loop\n");
    print_population_best(reserve_population);
  }
 ...
 private:
                          ...
                          bool consider_expensive_recombiners = true)
  {
    ...
    while (start_threshold_index >= 0) {
      printf("start_threshold_index: %d \n", start_threshold_index);
