...
template <typename i_t, typename f_t>
void solver_settings_t<i_t, f_t>::set_routing_callback(
    callbacks::base_routing_callback_t* callback)
{
  routing_callbacks_.push_back(callback);
}

template <typename i_t, typename f_t>
const std::vector<callbacks::base_routing_callback_t*> 
solver_settings_t<i_t, f_t>::get_routing_callbacks() const
{
